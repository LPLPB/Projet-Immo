...
function App() {
  ...
  useEffect(() => {
    if ((view === 'list' || view === 'ia') && session) fetchBiens();
  }, [view, session]);
  ...
  const handleSignUp = async () => {
    ...
    if (error) showNotify(error.message, 'error');
      ...
      showNotify('Compte créé ! Connectez-vous maintenant 🎉', 'success');
      setIsSignUp(false);
    }
  };
  ...
  const handleSignIn = async () => {
    ...
    if (error) showNotify(error.message, 'error');
      ...
      showNotify('Connecté avec succès ! 🚀', 'success');
  ...
  const migrateGuestData = async () => {
    ...
    if (guestBiens.length === 0) return;

    const toInsert = guestBiens.map((b) => ({
      nom: b.nom,
      loyer_mensuel: b.loyer_mensuel,
      charges_mensuelles: b.charges_mensuelles,
    ...
    const { error } = await supabase.from('biens').insert(toInsert);
    if (!error) {
      showNotify('Données migrées vers votre compte ! 🎊', 'success');
      fetchBiens();
    }
  };

  useEffect(() => {
    if (session && biens.some((b) => b.id.startsWith('guest_'))) {
      migrateGuestData();
    }
  }, [session]);
    ...
    <div style={appWrapper}>
        ...
        <div className="custom-modal" onClick={() => setShowSaveModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              <div style={{ padding: 12, backgroundColor: '#eff6ff', borderRadius: 16 }}>
                <Lock color="#3b82f6" />
              </div>
            ...
            <p style={{ fontSize: 13, color: '#64748b', marginBottom: 20 }}>
              {isSignUp
                ? 'Créez votre compte pour sauvegarder définitivement vos données.'
      ...
      {aiAnalysis && aiAnalysis !== 'loading' && (
        <div className="custom-modal" onClick={() => setAiAnalysis(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              <div style={{ padding: 12, backgroundColor: '#f5f3ff', borderRadius: 16 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 800, color: aiAnalysis.score === 'Profil Premium' ? '#10b981' : '#f59e0b' }}>
                {aiAnalysis.score}
              </div>
            </div>
            <div style={{ marginBottom: 20 }}>
      ...
      {aiAnalysis === 'loading' && (
        <div className="custom-modal">
          <div className="modal-content" style={{ textAlign: 'center', padding: 40 }}>
            <Loader2 className="animate-spin" size={48} color="#8b5cf6" style={{ margin: '0 auto 20px' }} />
            <b style={{ display: 'block', fontSize: 18 }}>L'IA analyse votre empire...</b>
        ...
        <div>
          <div style={disclaimerBox}>
            ...
            <span style={{ fontSize: 11, lineHeight: 1.5, color: '#78716c' }}>
              Ceci n'est pas un conseil en investissement. Je partage simplement le fruit de ma réflexion personnelle.
            </span>
          </div>
            ...
            <div className="guest-banner">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 13, color: '#fff', fontWeight: 700 }}>
                  Mode Invité - Données temporaires
                </span>
              </div>
              <button
          ...
          {view === 'menu' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <button onClick={() => setView('list')} className="menu-btn" style={menuBtnStyle}>
                <div style={{ padding: 12, backgroundColor: '#ecfdf5', borderRadius: 16 }}>
              ...
              <button onClick={() => setView('ia')} className="menu-btn" style={menuBtnStyle}>
                <div style={{ padding: 12, backgroundColor: '#f5f3ff', borderRadius: 16 }}>
          ...
          {view === 'add' && (
            <div style={formWrapper}>
              ...
              <h3 style={{ marginTop: 5, marginBottom: 25, fontSize: 20, fontWeight: 800 }}>
                Fiche Acquisition
              </h3>

              <div style={inputGroup}>
                <label style={labelStyle}>
                  Nom de l'actif <Tooltip text="Nom clair (ex: Studio Centre) pour identifier ce bien." />
                </label>
                <input
                ...
                <div style={{ display: 'flex', gap: 15 }}>
                  <div style={{ flex: 2 }}>
                    <label style={labelStyle}>
                      Ville <Tooltip text="Indispensable pour l'analyse des prix du marché par l'IA." />
                    </label>
                    <input
                      placeholder="Ville"
                      value={form.ville}
                  ...
                  <div style={{ flex: 1 }}>
                    <label style={labelStyle}>
                      Surface <Tooltip text="Surface habitable réelle en m²." />
                    </label>
                    <input
                      type="number"
                      placeholder="m²"
                ...
                <label style={labelStyle}>
                  Prix d'achat global (€) <Tooltip text="Prix net vendeur + frais de notaire + frais d'agence." />
                </label>
                <input
                  type="number"
                ...
                {form.loue && (
                  <div style={{ display: 'flex', gap: 15 }}>
                    <div style={{ flex: 1 }}>
                      <label style={labelStyle}>
                        Loyer HC <Tooltip text="Revenu mensuel hors charges." />
                      </label>
                      <input
                        type="number"
                        placeholder="Loyer"
                    ...
                    <div style={{ flex: 1 }}>
                      <label style={labelStyle}>
                        Charges <Tooltip text="Charges de copropriété par mois." />
                      </label>
                      <input
                        type="number"
                        placeholder="Charges"
                ...
                <div style={toggleBox}>
                  ...
                  <input
                    ...
                    style={{ width: 20, height: 20, cursor: 'pointer' }}
                    checked={form.credit}
                    onChange={(e) => setForm({ ...form, credit: e.target.checked })}
                  />
                </div>
                ...
                {form.credit && (
                  <div style={subForm}>
                    <label style={labelStyle}>
                      Capital Emprunté <Tooltip text="Montant total emprunté à la banque au départ." />
                    </label>
                    <input
                      type="number"
                      placeholder="Capital"
                      value={form.emprunt}
                      onChange={(e) => setForm({ ...form, emprunt: e.target.value })}
                      style={inputField}
                    />
                    <div style={{ display: 'flex', gap: 15, marginTop: 8 }}>
                      <div style={{ flex: 1 }}>
                        <label style={labelStyle}>
                          Mensualité <Tooltip text="Votre prélèvement mensuel prêt + assurance." />
                        </label>
                        <input
                          type="number"
                          placeholder="€ / mois"
                          value={form.mens}
                          onChange={(e) => setForm({ ...form, mens: e.target.value })}
                          style={inputField}
                        />
                      </div>
                      <div style={{ flex: 1 }}>
                        <label style={labelStyle}>
                          Déjà remboursé <Tooltip text="Montant du capital déjà amorti à ce jour." />
                        </label>
                        <input
                          type="number"
                          placeholder="Amorti"
                          value={form.remb}
                          onChange={(e) => setForm({ ...form, remb: e.target.value })}
                          style={inputField}
                        />
                      </div>
                    </div>
              ...
              <button onClick={handleSave} style={saveBtn}>
                {isGuest ? 'Ajouter (temporaire)' : 'Enregistrer l\'actif'}
              </button>
            </div>
          )}
          ...
          {view === 'list' && (
            <div style={formWrapper}>
              ...
              <div style={summaryCard}>
                <div>
                  <span style={{ fontSize: 10, opacity: 0.8, textTransform: 'uppercase', fontWeight: 800 }}>
                    Valeur Nette
                  </span>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
                {biens.map((b) => {
                  const cf = (b.loyer_mensuel || 0) - (b.charges_mensuelles || 0) - (b.mensualite || 0);
                  return (
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #f8fafc', paddingTop: 14, marginTop: 14 }}>
                        <div>
                          ...
                          <div style={{ fontWeight: 800, color: '#10b981', fontSize: 15 }}>
                            {(((b.loyer_mensuel * 12) / b.prix_achat) * 100 || 0).toFixed(1)}%
                          </div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <span style={{ fontSize: 10, color: '#94a3b8', fontWeight: 800, textTransform: 'uppercase' }}>
                          ...
                          <div style={{ fontWeight: 900, fontSize: 15, color: cf >= 0 ? '#3b82f6' : '#ef4444' }}>
                            {cf > 0 ? '+' : ''}{cf.toFixed(0)}€
                          </div>
                        </div>
                      </div>
          ...
          {view === 'ia' && (
            <div style={formWrapper}>
              ...
              <div style={{ ...bienCardStyle, backgroundColor: '#f5f3ff', border: '1.5px solid #ddd6fe', marginBottom: 20, padding: 20 }}>
                <label style={{ ...labelStyle, color: '#6b21a8', marginTop: 0, marginBottom: 8 }}>
                  Revenus Annuels Bruts (€) <Tooltip text="Entrez votre salaire annuel pour que l'IA calcule votre taux d'endettement réel." />
                </label>
                <input
                  type="number"
                  ...
                  style={{ ...inputField, backgroundColor: '#fff', borderColor: '#c4b5fd' }}
                />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15, marginBottom: 20 }}>
                <div style={{ ...bienCardStyle, textAlign: 'center', margin: 0 }}>
                  <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 800 }}>
                    ENDETTEMENT <Tooltip text="Limite conseillée : 35%." />
                  </div>
                  <div style={{ fontSize: 28, fontWeight: 900, color: parseFloat(tauxEndettement) > 35 ? '#ef4444' : '#10b981' }}>
                    {tauxEndettement}%
                  </div>
                </div>
                <div style={{ ...bienCardStyle, textAlign: 'center', margin: 0 }}>
                  <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 800 }}>
                    LOYERS MENSUELS <Tooltip text="Total de vos loyers bruts." />
                  </div>
              ...
              <div style={{ ...bienCardStyle, padding: 25, backgroundColor: '#fff', border: '1.5px dashed #e2e8f0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 15 }}>
                  <ShieldCheck size={24} color="#10b981" />
                  <b style={{ fontSize: 15 }}>Santé Financière</b>
                </div>
                <p style={{ fontSize: 12, color: '#64748b', lineHeight: 1.6, margin: 0 }}>
                  Votre cashflow net total est de <b>{(totalLoyers - totalMensualites).toFixed(0)}€/mois</b>.
              ...
              <button style={{ ...mainBtn, backgroundColor: '#8b5cf6', marginTop: 20 }} onClick={runAiAnalysis}>
...
const mainCard: any = {
  backgroundColor: '#fff', width: '92%', maxWidth: '440px', borderRadius: '32px',
  boxShadow: '0 25px 50px -12px rgba(0,0,0,0.1)', padding: '32px',
};
const inputField: any = {
  padding: '16px', borderRadius: '16px', border: '1.5px solid #e2e8f0', fontSize: '16px',
...
const summaryCard: any = {
  display: 'flex', justifyContent: 'space-between', background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)',
  padding: '28px', borderRadius: '28px', color: '#fff', marginBottom: 28,
};
const bienCardStyle: any = {
  padding: '22px', borderRadius: '24px', border: '1.5px solid #f1f5f9', backgroundColor: '#fff', marginBottom: 15,
};
const toggleBox: any = {
  display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px',
  backgroundColor: '#f8fafc', borderRadius: '18px', marginTop: 15, border: '1.5px solid #f1f5f9',
};
const subForm: any = {
  display: 'flex', flexDirection: 'column', gap: '10px', padding: '20px',
  borderLeft: '4px solid #3b82f6', backgroundColor: '#f8fafc', marginTop: 12, borderRadius: '0 20px 20px 0',
};
...
const headerStyle: any = {
  display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 35,
};
const logoutBtn: any = {
  background: '#f1f5f9', border: 'none', padding: '12px', borderRadius: '15px', cursor: 'pointer', color: '#94a3b8',
};
const backBtn: any = {
  background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', display: 'flex',
  alignItems: 'center', gap: 8, marginBottom: 20, fontWeight: 800, padding: 0, fontSize: 15,
};
const toastStyle: any = {
  position: 'fixed', top: '24px', left: '50%', transform: 'translateX(-50%)', padding: '16px 32px',
  color: '#fff', borderRadius: '100px', zIndex: 99999, fontWeight: 800, fontSize: 14,
  display: 'flex', gap: 12, alignItems: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.2)',
};
const inputGroup: any = { display: 'flex', flexDirection: 'column' };
const formWrapper: any = {};
const menuBtnStyle: any = {
  transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', border: '1.5px solid #f1f5f9',
  background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 18,
  padding: 22, borderRadius: 24, width: '100%', outline: 'none',
};
const disclaimerBox: any = {
  display: 'flex', gap: 10, alignItems: 'flex-start', padding: '14px 18px',
  backgroundColor: '#fef3c7', border: '1.5px solid #fde68a', borderRadius: '16px', marginBottom: 20,
};
